...
- GitHub Actions `desktop-client.yml` 的 macOS job 安装 `autoconf`、`automake`、`libtool`、`nasm` 和 `pkg-config`。
- GitHub Actions `desktop-client.yml` 的 macOS job 显式关闭 Flutter Swift Package Manager；`pubspec.yaml` 的 `flutter.config.enable-swift-package-manager` 固定为 `false`。
...
- 生成 `desktop-client/build/macos/Build/Products/Release/FrameLean-v1.2.1.dmg`。
