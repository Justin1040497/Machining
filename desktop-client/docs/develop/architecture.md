...
tools/
dependencies/
build/dependencies/  本地生成运行时，不进入版本库
legal/
docs/
.workspace/  根级本地 ignored 工作区，不进入版本库
```

当前验证和发布平台是 macOS Universal 2（Intel x86_64 + Apple Silicon arm64）与 Windows x64。Linux 和 Web 不在当前支持范围，仓库不保留对应平台工程。
