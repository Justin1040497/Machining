...
-- FrameLean self-hosted update module on RuoYi-Vue-Plus.
-- This migration is safe for existing v1 databases and for fresh installs.
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_releases_version_build
    ON releases (version, build_number);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_release_packages_platform
    ON release_packages (release_id, platform, arch);

CREATE INDEX IF NOT EXISTS idx_release_packages_client_lookup
    ON release_packages (release_id, platform, client_visible);
...
CREATE INDEX IF NOT EXISTS idx_download_events_release
    ON download_events (release_id);
...
CREATE TABLE IF NOT EXISTS admin_auth_config (
    ...
    cipher_algorithm      VARCHAR(32) NOT NULL,
...
COMMENT ON TABLE admin_auth_config IS 'Deprecated FrameLean v1 admin auth envelope; RuoYi/Sa-Token is authoritative.';

CREATE TABLE IF NOT EXISTS release_artifact_requirements (
    id          BIGSERIAL   PRIMARY KEY,
    ...
    created_at  TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT uk_release_artifact_requirement UNIQUE (release_id, platform, arch)
);

CREATE INDEX IF NOT EXISTS idx_release_artifact_requirements_release_id
    ON release_artifact_requirements (release_id);

CREATE TABLE IF NOT EXISTS update_check_events (
    id               BIGSERIAL    PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_update_check_events_created_at
    ON update_check_events (created_at DESC);

CREATE INDEX IF NOT EXISTS idx_update_check_events_ip_address
    ON update_check_events (ip_address);

CREATE INDEX IF NOT EXISTS idx_update_check_events_release_id
    ON update_check_events (release_id);
...
CREATE INDEX IF NOT EXISTS idx_ip_block_rules_ip_enabled
    ON ip_block_rules (ip_address, enabled);

CREATE INDEX IF NOT EXISTS idx_ip_block_rules_created_at
    ON ip_block_rules (created_at DESC);

-- Hide upstream routes that are not part of the FrameLean admin surface.
UPDATE sys_menu
SET visible = '1', status = '1'
WHERE menu_id IN (3, 4, 5, 6, 115, 117, 120, 121, 122, 1500, 1506, 11616, 11618, 11638, 11700, 11701);

INSERT INTO sys_menu VALUES
    (2000, 'FrameLean', 0, 1, 'framelean', NULL, '', 1, 0, 'M', '0', '0', '', 'rocket', 103, 1, now(), NULL, NULL, 'FrameLean 更新管理目录')
...
INSERT INTO sys_menu VALUES
    (2001, '发布版本', 2000, 1, 'releases', 'framelean/releases/index', '', 1, 0, 'C', '0', '0', 'framelean:release:list', 'upload', 103, 1, now(), NULL, NULL, 'FrameLean 发布版本管理'),
    (2002, '更新审计', 2000, 2, 'audit', 'framelean/audit/index', '', 1, 0, 'C', '0', '0', 'framelean:audit:list', 'log', 103, 1, now(), NULL, NULL, 'FrameLean 更新检查与下载审计'),
    (2003, '运行诊断', 2000, 3, 'diagnostics', 'framelean/diagnostics/index', '', 1, 0, 'C', '0', '0', 'framelean:diagnostics:list', 'monitor', 103, 1, now(), NULL, NULL, 'FrameLean COS Redis 与公网地址诊断')
ON CONFLICT (menu_id) DO UPDATE SET
    menu_name = EXCLUDED.menu_name,
    parent_id = EXCLUDED.parent_id,
    order_num = EXCLUDED.order_num,
...
INSERT INTO sys_menu VALUES
    (2010, '发布版本查询', 2001, 1, '#', '', '', 1, 0, 'F', '0', '0', 'framelean:release:query', '#', 103, 1, now(), NULL, NULL, ''),
    (2011, '发布版本新增', 2001, 2, '#', '', '', 1, 0, 'F', '0', '0', 'framelean:release:add', '#', 103, 1, now(), NULL, NULL, ''),
    (2012, '发布版本修改', 2001, 3, '#', '', '', 1, 0, 'F', '0', '0', 'framelean:release:edit', '#', 103, 1, now(), NULL, NULL, ''),
    (2013, '发布版本上线', 2001, 4, '#', '', '', 1, 0, 'F', '0', '0', 'framelean:release:publish', '#', 103, 1, now(), NULL, NULL, ''),
    (2014, '发布版本删除', 2001, 5, '#', '', '', 1, 0, 'F', '0', '0', 'framelean:release:remove', '#', 103, 1, now(), NULL, NULL, ''),
    (2020, '审计查询', 2002, 1, '#', '', '', 1, 0, 'F', '0', '0', 'framelean:audit:query', '#', 103, 1, now(), NULL, NULL, ''),
    (2030, '诊断查询', 2003, 1, '#', '', '', 1, 0, 'F', '0', '0', 'framelean:diagnostics:query', '#', 103, 1, now(), NULL, NULL, '')
ON CONFLICT (menu_id) DO UPDATE SET
    menu_name = EXCLUDED.menu_name,
    parent_id = EXCLUDED.parent_id,
    order_num = EXCLUDED.order_num,
...
SELECT 3, id
FROM (VALUES (2000), (2001), (2002), (2003), (2010), (2011), (2012), (2013), (2014), (2020), (2030)) AS menu(id)
ON CONFLICT DO NOTHING;
-- Tighten FrameLean single-tenant update admin scope.

-- Keep only FrameLean update management plus the minimum RuoYi shell needed for
-- users, roles, menus, and login / operation logs.
UPDATE sys_menu
SET visible = '1', status = '1'
WHERE menu_id NOT IN (
    1, 100, 101, 102, 108, 130, 131, 500, 501,
    1001, 1002, 1003, 1004, 1005, 1006, 1007,
    1008, 1009, 1010, 1011, 1012,
    1013, 1014, 1015, 1016,
    1040, 1041, 1042, 1043, 1044, 1045, 1050,
    2000, 2001, 2002, 2003, 2010, 2011, 2012, 2013, 2014, 2020, 2030
);
...
-- Single-person admin: password login only, no register / SMS / email / xcx /
-- social grants.
UPDATE sys_config
...
UPDATE sys_client
SET grant_type = 'password', device_type = 'pc', status = '0', update_time = now()
WHERE del_flag = '0';

UPDATE sys_dict_data
SET status = '1'
WHERE dict_type = 'sys_grant_type'
  AND dict_value IN ('sms', 'email', 'xcx', 'social');

UPDATE sys_dict_data
SET status = '1'
WHERE dict_type = 'sys_device_type'
  AND dict_value IN ('android', 'ios', 'xcx');

-- Historical releases without the current auto-update payload matrix must not
-- be eligible for /latest.
UPDATE releases
SET status = 'archived',
    published_at = NULL,
    updated_at = now()
WHERE status = 'published'
  AND (
    NOT EXISTS (
        ...
        WHERE release_packages.release_id = releases.id
          AND platform = 'windows-installer'
          AND arch = 'x64'
