...
class DefaultFfmpegCommandBuilder implements FfmpegCommandBuilder {
  ...
  String resolveVideoEncoder({
    ...
    try {
      ...
      throw FfmpegCommandBuildException(
        '${videoCodecLabel(targetCodec)} 不能使用 ${encoderBackendLabel(backend)} 编码器',
      );
    } on UnsupportedEncoderBackendException catch (error) {
      throw FfmpegCommandBuildException(
        '当前 FFmpeg 不支持 ${encoderBackendLabel(error.backend)} 编码器: ${error.encoderName}',
      );
    }
  }
  ...
  String buildLogHint(
    ...
    return switch (task.purpose) {
      ...
      TaskPurpose.conversion =>
        '使用 ${videoCodecLabel(targetCodec)} / $videoEncoder 路线生成目标封装格式文件',
    };
  }
  ...
  String buildCompressionLogHint(
    ...
    if (recommendation.profile == CompressionProfile.extreme ||
      ...
      return '$strategy，目标分辨率 ${resolutionPresetLabel(resolutionPreset)}，'
          '目标编码 ${videoCodecLabel(targetCodec)} / $videoEncoder，'
          '目标视频码率 ${formatBitrate(recommendation.targetVideoBitrate!)}，'
          '目标音频码率 ${formatBitrate(recommendation.targetAudioBitrate!)}';
    }

    return '${recommendation.message}，'
        '使用 ${videoCodecLabel(targetCodec)} / $videoEncoder CRF ${recommendation.crf} '
        '生成输出文件';
  }

  String videoCodecLabel(VideoCodec codec) {
    return switch (codec) {
  ...
  String encoderBackendLabel(EncoderBackend backend) {
