...
class FrameLeanSliverReorderableListState
  ...
  FrameLeanReorderGapDetails _dragUpdateItems() {
    ...
    for (final _ReorderableItemState item in _items.values) {
      ...
      final double itemStart = _scrollDirection == Axis.vertical
          ? geometry.top
          : geometry.left;
  ...
  void _restoreGapToDragOrigin(double gapExtent) {
    final dragIndex = _dragIndex!;
    if (_insertIndex == dragIndex) {
      return;
    }
    _insertIndex = dragIndex;
    for (final _ReorderableItemState item in _items.values) {
      if (item.index == dragIndex || !item.mounted) {
        continue;
      }
      item.updateForGap(dragIndex, dragIndex, gapExtent, true, _reverse);
    }
  }
  ...
  Widget _wrapWithSemantics(Widget child, int index) {
    ...
    if (index < widget.itemCount - 1) {
      ...
      semanticsActions[CustomSemanticsAction(label: reorderItemAfter)] =
          moveAfter;
      semanticsActions[CustomSemanticsAction(
            label: localizations.reorderItemToEnd,
          )] =
          moveToEnd;
    }
    ...
    if (newTargetOffset != _targetOffset) {
      ...
      if (animate) {
        if (_offsetAnimation == null) {
          ...
          final double currentAnimValue = Curves.easeInOut.transform(
            _offsetAnimation!.value,
          );
...
typedef _DragItemUpdate =
    void Function(_DragInfo item, Offset position, Offset delta);
