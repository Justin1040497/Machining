...
## 2026-07-15｜v1.2.1｜No Release

按 FrameLean 手绘箭头参考图重制工作台背景引导箭头，在保留动态定位、限长、安全裁剪和移动动画的前提下，提高铅笔复描、S 形曲线和排线箭头头的辨识度。

### Changed

- 应用构建号由 `7` 升级到 `8`，版本保持 `1.2.1`；同步重新生成应用内构建信息，确保桌面包版本、更新比较和发布元数据使用同一构建号。
- 箭头主体改为沿目标方向单调前进的双段三次贝塞尔曲线，使用 3.8px 圆角主线、偏移复描和 seeded 短纹理模拟铅笔重复描边；三条箭头统一参考全部开始箭头的自然单弧比例和曲率强度 72，只根据目标方向翻转弧面。任务操作与全部开始箭头最大长度为 245px、实际起终点距离约 190～215px；添加按钮箭头独立缩短到约 145～165px，最大长度为 180px。
- 箭头头改为约 23～30px 的不对称封闭三角形，增加低透明填色、三道内部排线和短终点强调线；不绘制容易被误认为反向箭头的起点装饰，所有附加笔画继续受各内容组安全裁剪区约束。
- 三个内容组显式指定箭头进入目标点的方向：任务箭头向上，全部开始与添加按钮箭头向下；三条线统一使用短三次贝塞尔自然连接到真实凹口正中，不使用独立直线收束段，避免方钩、折角和沿翼边进入。
- 任务操作箭头按工作台宽度采用 185～220px 的响应式水平跨度；任务操作和全部开始文案跟随各自箭尾并保留 14px 视觉间距。空队列页将箭头层与内容层分离，导入图标和三行说明作为一个整体居中显示，左下角仅保留独立短箭头。所有引导文案不使用逗号，需要停顿时以空格替代。
- 三条工作台箭头统一使用低饱和灰色 42% 透明度，Guide 场景根层的可见透明度统一降至 50%，使提示文字、图标和箭头整体后退；不改变目标锚点、Guide Scheduler 或场景动画时长。
- 队列中已有任务时，双击任务或任务夹之外的空白背景也会打开现有导入入口；原始指针监听只识别主按钮和空白区域，不抢占任务卡片点击、列表拖拽或框选手势。

### Fixed

- 修复多任务场景中“任务操作”和“全部开始”两组背景引导相互交叉、文字压线的问题：最后一张任务卡与开始按钮之间的可用空间统一划分为上下两条安全通道，中间固定保留 28px；空间达到 360px 时显示两组，180～359px 时只保留“全部开始”，不足 180px 或列表可滚动时隐藏。箭身、箭头头、强调线和文本的最终视觉边界会在绘制前校验，双引导无法安全放置时自动降级，不缩短或压平现有箭头。

### Verified

- 工作台 Guide、布局、Scheduler 与背景双击定向测试 27 项通过，包含 360 / 180px 容量边界、1370px 单任务回归、双通道隔离、降级显示、窗口变化状态保持、箭头几何，以及有任务时背景双击导入和前景任务排除；Guide 作用域静态分析无 issue。
- 通过 1100×760 浅色与深色固定预览检查任务操作和全部开始箭头位于独立安全通道，文本按指定位置换行且视觉边界不相交。
- `flutter test` 445 项全部通过；全量 `flutter analyze` 无新增 warning / error，仍只有主线已有的 5 项 info 级提示。
