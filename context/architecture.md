...
`fengine` 是独立 Cargo 工程，通过本地 path 依赖使用实际需要的 `fll` crates，不是 `fll` workspace member。FEngine 已实现分析/执行批次、队列 revision 和原子重排、Engine Snapshot、进度与执行控制；FLL Runtime 实现真实 execution、输出事务、单 lane 安全暂停和 LIFO 抢占恢复。Desktop Client 的全部标准媒体路径统一经 Engine Gateway，不保留 Dart FFmpeg Runner、FFprobe 分析器或 FFmpeg 命令规划。当前真实 Backend 只执行不含转换节点的 libav packet stream-copy/remux；完整压缩/转码 Pipeline 仍 fail closed。Backend 内部模块以 `backend/pom.xml` 为唯一事实来源。
