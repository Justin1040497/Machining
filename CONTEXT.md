...
- 工作台背景层会按队列状态显示轻量动态提示：空队列把居中的导入图标与三行说明作为独立内容层，并用另一条短箭头指向左下角添加按钮；有任务且列表未滚动时，最后一张任务卡与开始按钮之间达到 360px 会在互不重叠的上下安全通道显示任务操作和全部开始提示，180～359px 只保留全部开始，不足 180px 时隐藏。空队列和有任务时均可双击未被任务或任务夹占用的背景区域打开导入入口。引导层使用 Canvas 手绘箭头、动态锚点测量和场景淡入淡出，不覆盖或拦截任务列表、顶部栏和底栏交互。
