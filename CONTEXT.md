...
| `v1.2.1` | 自托管更新客户端体验接入、媒体处理可靠性修复和任务夹交互重构：更新入口 / 下载 / helper、自托管服务、图片有效压缩验收、透明视频保留、输出 preflight、批量导入任务夹 |
...
- 工作台从 v1.2.1 开始把任务夹作为本地持久化实体：批量导入会按媒体类型自动建夹，总列表显示任务夹和未入夹任务；任务夹主体打开夹级配置，尾部查看按钮打开左侧内容浮层，夹内任务可按普通任务行启动、暂停、重试、重链、查看日志或移出。
- 工作台任务夹支持批量工作流：未入夹任务可多选后按媒体类型创建任务夹，也可从拖拽手柄拖入同类型任务夹；任务夹尾部主按钮可按夹内状态批量暂停、启动下一项或重试终态任务。
