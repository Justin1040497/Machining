...
- 工作台导入入口支持文件和文件夹；文件夹按设置中的扫描深度递归查找可识别媒体，默认扫描到第 2 层。识别到 1 个媒体时创建普通任务，多个媒体时按视频、图片、音频分别创建以源文件夹命名的任务夹。
- 任务详情以“压缩 / 格式转换”作为处理意图，高级设置作为附加配置默认收起，当前包含线程限制、视频两遍压缩模式和多音频流选择；用户线程上限会参与并行资源预算。
...
- 队列执行从 v1.2.1 开始支持受控并行：应用设置保存用户期望的最大并行任务数（默认 2，上限 3），实际执行位由本机资源守卫按 CPU、内存和运行任务类型降级；点击任务开始仍是插队入口，空闲时立即执行，否则等待最早空出的执行位。
