...
- 工作台任务夹支持批量工作流：未入夹任务可多选后按媒体类型创建任务夹；任务经过同类型任务夹整行时冻结排序 gap，释放在中部主体会入夹，释放在上下 16px 边缘仍排序，离开任务夹行到普通任务上才恢复排序预览；排序释放后以乐观视觉顺序等待持久化确认，不短暂返回旧位置；跨类型任务夹在拖起时禁用显示并作为排序目标处理。任务夹尾部主按钮可按夹内状态批量暂停、启动下一项或重试终态任务；空任务夹会自动删除。
