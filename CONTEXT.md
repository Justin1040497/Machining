...
- 工作台任务夹支持批量工作流：顶层和夹内列表统一使用全应用 `FrameLeanReorderableListView`；未入夹任务经过同类型任务夹整行时冻结 gap，中部释放入夹，上下 16px 边缘继续排序；夹内任务可拖到面板外遮罩移回总列表尾部，释放后在落点缩小淡出，失败时恢复。两层排序都以乐观觉顺序等待持久化；跨类型任务夹禁用显示并作为排序目标。任务行不再显示选中边框，多选仅由复选框表达。空任务夹会自动删除。
