...
FrameLean 更新服务端位于 Monorepo 的 `backend/`，使用 RuoYi-Vue-Plus 5.X + plus-ui 5.X、Java Spring Boot、PostgreSQL、Flyway、MyBatis-Plus、Redis 和腾讯云 COS。`ruoyi-admin` 是入口模块，`ruoyi-modules/ruoyi-framelean` 保存更新业务，`admin-web` 提供后台页面。
