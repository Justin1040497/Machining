...
Monorepo 的 `backend/` 使用 RuoYi-Vue-Plus 5.X + plus-ui 5.X。`ruoyi-admin` 是后端入口，`ruoyi-modules/ruoyi-framelean` 承载更新业务，`admin-web` 承载管理页面。RuoYi 负责账号密码登录、菜单、角色、权限和操作日志；`/api/v1/admin/**` 同时接受 RuoYi / Sa-Token 登录态和脚本用 `X-Api-Key`。
