...
- 普通模式下未入夹任务的拖拽柄同时承担排序和入夹：拖到同类型任务夹主体会入夹，拖到任务夹边缘或普通任务上仍按总列表排序；不同媒体类型任务夹在拖起时禁用显示并作为排序目标处理。普通模式也支持框选，命中任务后进入多选；Command / Control 框选按反选处理。
...
- 主列表多选 FAB 可以按媒体类型创建任务夹；未入夹任务可通过拖拽柄拖入同类型任务夹主体，拖到任务夹边缘或普通任务上仍排序，跨类型任务夹在拖起时禁用显示并作为排序目标处理。
