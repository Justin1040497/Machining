...
- 最大并行任务数默认 2，可在应用设置分区选择 1 到 3，保存、取消和 Drift 持久化读取都应归一化到合法范围。
- 文件夹导入扫描深度默认 2，可选 0 到 5，设置页提示扫描层级越深耗时越长，并覆盖保存、取消和 Drift 往返。
- 通知策略默认区分持久通知、临时通知和不通知；任务结果 / 更新 / 重要失败默认持久，保存成功、复制成功和普通交互默认临时。
- 快捷键绑定默认包含 `F` 添加文件、`Space` 开始 / 暂停、主修饰键 + `,` 打开设置、主修饰键 + `Shift+N` 打开通知中心；支持重映射、冲突检测和恢复默认。
- 关闭行为默认最小化到后台，可切换为退出应用；Windows 需要覆盖托盘恢复和显式退出，macOS 需要覆盖 Dock 重新打开隐藏窗口。
- 底部栏设置入口和新任务默认配置应用。

### Application Use Cases

- 单文件和批量导入共享同一任务创建规则：批量读取设置、识别视频 / 图片 / 音频媒体类型后，在单次写入中先持久化为 `awaitingAnalysis`；分析队列真正取得执行位时才转换为 `analyzing`，成功后才进入 `pending`。
- 文件夹扫描遵守第 0 层为根目录的深度定义，不跟随目录符号链接；不支持文件静默跳过，读取失败进入扫描问题汇总。
- 文件夹识别到 0 个媒体时不创建任务；每个导入批次按媒体类型独立计数，某类型 1 个时创建普通任务，2 个及以上才创建该类型任务夹，同名任务夹追加序号。
- 输出设置保存后只批量刷新等待中、失败和已取消任务，运行中、已暂停和已完成任务保留当前执行快照。
- 单任务和任务夹配置支持 `system`、`source`、`custom` 输出位置模式；`system` 在执行时读取最新应用输出设置，`source` 和 `system` 互斥，只有二者都关闭时才能填写自定义目录。
...
- 任务夹开始按 `folderSortOrder` 填充可用执行位，后续只补夹内任务；夹内单任务开始仍使用一次性任务 / 抢占语义。
- 新任务和新任务夹插入时应位于未完成顶层项之后、已完成顶层项之前，不破坏普通任务和任务夹的混排顺序。
- 队列启动、单任务开始 / 继续、暂停和清空时取消执行；清空会同步清空任务和任务夹。
- 受控并行执行按设置读取用户上限，并通过资源守卫按 CPU、内存和任务类型降级；视频重任务不能把所有执行位挤满导致设备卡死。
- 预览帧生成通过 `GeneratePreviewFramesUseCase` 读取运行时并调用预览服务。
- 应用通知按事件策略投递；持久通知写入通知中心并展示，临时通知只展示浮层，不通知则完全抑制；FFmpeg 队列完成 / 失败直接产生类型化任务通知。
- FFmpeg 输出保护覆盖隐藏 partial 创建、执行中外部删除检测、暂停保留、取消 / 失败 / 异常退出清理、成功发布前重名避让和应用启动后的遗留 partial 清理。
...
- 通知设置页按事件选择“通知 / 临时通知 / 不通知”；设置保存成功、复制成功和普通交互切换为临时通知时不应进入通知中心。
- 版本更新通知按 `dedupe_key` 保证一个版本只展示一条；当前更新通知点击打开 L2 更新通知弹窗，历史更新通知进入 L3 版本日志页，载荷中的 GitHub / Gitee / 备用地址能恢复对应下载动作。
...
- `Esc` 固定关闭最上层弹窗、通知中心、任务夹侧栏或返回上一页，工作台根页面无操作。
- 输入框聚焦时抑制 `F`、`Space` 等普通无修饰键快捷键，但保留主修饰键快捷键。
- 快捷键设置页记录新组合时应检测与其他动作冲突；恢复默认后保存并在工作台立即生效。
- 关闭到后台时不终止运行中任务；显式退出且存在运行或暂停任务时必须确认，确认后终止任务并清理 partial 输出。
...
- 图片无损压缩开关和成功任务 `output_file_size` 能在 JSON / Drift 行和 domain 之间往返；旧数据缺失字段时保持兼容默认值。
...
- 图片无损模式只生成 PNG、WebP 或 TIFF 输出；WebP 命令包含 lossless 与最高质量参数，TIFF 使用 Deflate，且不得降级到 JPG。
...
- 任务行开始按钮作为插队入口；满执行位时暂停实际运行最早的任务，连续插队按 FIFO 恢复被抢占任务且不修改列表排序。
- FFmpeg 不可用时的失败状态。
- 极限压缩确认拦截。
...
- 硬件编码器会话失效命中可恢复特征时，当前 step 只自动重试一次，重复失败后进入普通失败流程并保留原始日志。
...
- 进程长时间无 stdout / stderr 活动时应被强制终止并标记为无响应超时，不能让任务永久占用执行位。
- FFprobe 分析超时时必须终止底层子进程并回收输出流，避免批量导入损坏文件或网络盘卡顿后泄漏进程。
...
- 任务夹内容浮层从左侧滑入，动画、遮罩和 `Esc` 关闭语义与通知中心右侧浮层对称；夹内任务复用普通任务行样式，可以启动、暂停、重试、重链、查看日志、排序和移出，且任务主体点击不会打开配置弹窗。
...
- 多选模式显示复选框，底部多选按钮使用选择语义图标；选中未入夹任务后在应用左侧显示批量操作条，并按媒体类型拆分创建任务夹。
...
- 顶部栏主题按钮可在浅色和深色之间切换。
- 任务和任务夹右键菜单使用项目圆角菜单风格；任务菜单的“添加到任务夹”二级菜单只列出当前媒体类型、可加入的现有任务夹。
...
- 点击底部 `+` 可选择导入文件或导入文件夹；拖拽文件夹走同一递归扫描和分组规则。
- 导入不支持的文件扩展名时显示只能导入媒体文件的提示。
- 大写 `.MOV` 后缀视频可以导入并开始压缩；如果输出文件名只和源文件大小写不同，应自动追加中文括号后缀。
- 大写图片和音频扩展名可以识别为对应媒体类型。
- 文件选择、文件拖入和文件夹扫描使用同一批次分组规则：每种媒体独立计数，同类型 1 个保持普通任务，2 个及以上创建一个任务夹。

### 媒体分析和任务列表

- 新任务先进入“等待分析”，分析队列取得执行位后进入“分析中”，分析结果持久化后进入“等待开始”；快速分析完成时任务列表也必须立即刷新，不得永久停留在旧状态。
- 分析成功后显示源文件信息。
- 分析失败后显示错误状态和顶部通知。
...
- 图片压缩任务可开启无损压缩；开启后格式列表只保留 PNG、WebP、TIFF。质量滑杆只对有损 JPG / WebP 显示；格式转换用途不显示质量、无损开关或分辨率。
- 图片、视频和音频任务配置面板可修改分类型处理配置；非视频任务打开任务详情时不得读取视频专属编码器状态。
- 图片、视频和音频任务都可配置元数据保留策略，三类媒体默认均保留元数据。
- 任务详情顶部只将“压缩 / 格式转换”作为意图选择；三类媒体的格式转换主区域都只显示目标格式，元数据和线程限制位于高级设置，两遍压缩只在视频压缩用途显示。

### 任务夹交互

- 任务夹作为总列表项显示，夹内任务不在总列表重复出现。
- 点击任务夹主体打开夹级配置，保存后应用到夹内可安全更新的任务。
- 任务夹配置顶部显示任务数、总源体积、格式分布和总时长 / 图片尺寸分布，不显示任何单个文件摘要；保持原格式按每个夹内源文件分别解析。
- 点击任务夹尾部查看按钮打开左侧内容浮层；浮层从左侧滑入，显示夹名、任务数和夹内任务列表。
...
- 主列表左侧批量操作条可以按媒体类型创建任务夹；未入夹任务经过同类型任务夹整行时不换位，释放中部主体时入夹，释放上下 16px 边缘时排序，离开任务夹行后才恢复排序预览；跨类型任务夹在拖起时禁用显示并作为排序目标处理。
- 新建任务夹使用媒体类型加序号命名；右键任务夹可重命名，重命名为空时显示提示。
...
- 点击单个任务的开始按钮能执行该任务。
- 点击底部主按钮能启动队列。
- 底部主按钮按总列表顺序执行，遇到任务夹时按夹内顺序展开执行；队列会按当前有效执行位并行填充可执行任务。
- 执行中点击某个任务开始会插队：有空闲且资源守卫允许时立即执行，无空位时暂停最早运行者并立即启动点击任务；被抢占任务按 FIFO 优先于普通候选恢复。
- 单任务开始本身不激活连续队列；任务完成且没有被抢占任务时不启动其他内容。任务夹开始只连续执行夹内任务，底部开始才按总列表展开顺序连续执行。
- 底部暂停清除连续作用域并暂停全部运行任务；暂停后单独开始任务不会恢复旧队列。
- 音频压缩输出不小于源文件时任务失败并清理无效输出；音频格式转换不执行体积验收。
- 运行中任务显示进度。
- 运行中任务可以暂停。
- 暂停任务可以继续。
- 任务可以取消、删除和重试。
...
- 关于栏更新按钮在 `检查更新`、`检查中` 和发现更新状态间切换时保持固定尺寸；默认 release 在 L2 弹窗展示实际存在的 GitHub / Gitee / 备用下载按钮，并通过系统浏览器打开。
- 工作台顶部在存在更新、下载中、暂停或待重启时显示 L1 状态胶囊；下载中显示迷你进度，点击打开 L2 更新通知弹窗。
- 非 mandatory 更新在 L2 点击「下次再说」后，同版本后续自动检查不再自动弹窗；手动入口仍会展示弹窗，新版本会自动取消 snooze。
- release 同时返回外部地址和 package 元数据时，客户端必须优先外部地址，不查找本地 package、不创建 download ticket、不启动下载。
- 只返回完整 package 元数据时，保留路线继续显示下载百分比、暂停 / 继续和安装动作；Windows 下载完成待重启时先确认任务状态再启动 updater helper，macOS 下载完成后打开应用私有缓存中的 DMG 所在位置。

### 完成和结果处理

- 处理完成后不弹出完成提示弹窗，完成反馈只通过完成提示音、临时通知、通知中心和任务项尾部文件入口表达。
- 已完成且存在 `outputPath` 的任务项尾部显示完成文件入口，点击后打开 Finder、Explorer 或文件管理器并定位输出文件。
- 已完成压缩任务显示源体积、输出体积和压缩比例；已完成格式转换任务显示源格式和目标格式，不显示压缩百分比。
...
- 右键任务可以打开文件所在位置。
- 右键任务可以重命名。
- 右键任务可以通过“添加到任务夹”二级菜单加入同媒体类型任务夹。
- 右键任务可以查看 FFmpeg 执行日志。
- 重命名为空时显示提示。
- 右键任务可以删除。
- 右键任务夹可以重命名、打开夹内任务、查看聚合日志或删除。

### macOS 构建验证

- GitHub Actions `build-macos.yml` 安装 `autoconf`、`automake`、`libtool`、`nasm` 和 `pkg-config`。
- GitHub Actions `build-macos.yml` 显式关闭 Flutter Swift Package Manager；`pubspec.yaml` 的 `flutter.config.enable-swift-package-manager` 固定为 `false`。
- `macos/` 工程必须保留 `Podfile`、`Podfile.lock`、Runner workspace 的 `Pods.xcodeproj` 引用、`Pods-Runner` xcconfig include 和 `[CP]` Build Phase。
- `scripts/release/build_dmg_macos.sh` 使用 UTF-8 locale，并在 `flutter build macos --release` 前后检查 CocoaPods 工程引用、Universal FFmpeg、包内运行时和法律资料。
...
- `FrameLeanUpdaterHelper.exe` 继续随包提供；只有服务端启用 package 路线且客户端完成下载与验签后才启动 helper。外部下载地址模式不直接调用 helper。
