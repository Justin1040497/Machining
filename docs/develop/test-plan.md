...
- 版本更新通知按 `dedupe_key` 保证一个版本只展示一条；当前更新通知点击打开 L2 更新通知弹窗，历史更新通知进入 L3 版本日志页，通知项仍提供“查看版本日志”和“下载更新”文字按钮。
...
- 关于栏更新按钮在 `检查更新`、`检查中`、下载百分比、暂停 / 继续和安装动作状态间切换时保持固定尺寸；Windows 文案为 `现在更新` / `重启更新`，macOS 手动 DMG 文案为 `下载 DMG` / `打开 DMG`。
...
- Windows 下载完成待重启时，如果存在运行中、暂停中、等待中或分析中任务，先显示项目风格警告弹窗，确认后暂停任务并启动 updater helper；macOS 下载完成后打开 DMG 所在位置，不触发重启前任务暂停。
